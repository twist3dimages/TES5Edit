...
var
  ...
  SetDateTime   : TDateTimeInfoRec;
  theFile       : string;
  Err           : Cardinal;
  ok            : Boolean;

    function ForceDateModified(theFile: String; NextDateTime: TDateTime): Boolean;
    var
      theHandle      : THandle;
      theInfo        : TByHandleFileInformation;
    ...
    begin
      Result := False;
      theHandle := CreateFile(PWideChar(theFile),
        GENERIC_WRITE,
        FILE_SHARE_WRITE,
        nil,
        OPEN_EXISTING,
      ...
      if (theError=0) and (theHandle<>0) then begin
        fileDate := DateTimeToFileDate(NextDateTime);
        FileSetDate(theFile, fileDate);
        if GetFileTime(theHandle, @CreationTime, @LastAccessTime, @LastWriteTime) then begin
          LastWriteTime.dwLowDateTime  := LongRec(fileDate).lo;
          LastWriteTime.dwHighDateTime  := LongRec(fileDate).hi;
          if SetFileTime(theHandle, @CreationTime, @LastAccessTime, @LastWriteTime) then begin
            GetFileInformationByHandle(theHandle, theInfo);
            Result := True;
          end;
        end;
...
begin
  if Application.MessageBox(PWideChar('Do NOT use with Steam workshop mods! '+#13#10#09+'Continue ?'), PWideChar('Warning'), MB_YESNO) = IDYES then begin
    NextDateTime := 0;
    for i := 0 to Pred(theList.Count) do begin
      theFile := ExpandFileName(wbDataPath+'\'+theList[i]);
      ...
      if FileExists(theFile) then begin
        ok := ForceDateModified(theFile, NextDateTime);
        Err := GetLastError;
        if not ok or (Err <> 0) then
          frmMain.PostAddMessage(theFile+' could not be changed. Error: '+IntToStr(Err))
        else begin
          ok := FileGetDateTimeInfo(thefile, SetDateTime);
          if ok then begin
            ok := SecondsBetween(SetDateTime.TimeStamp, NextDateTime)<2;
          end;
          if not ok then
            frmMain.PostAddMessage(theFile+' did not change!');
        end;
      end;
    end;
  end;
...
//              end;
      ...
      end else if wbBuildRefs and (GetKeyState(VK_CONTROL) < 0) then begin
        wbForceBuildRefs := True;
        AddMessage('The CONTROL key is pressed, force building references for all plugins!');
      end;

      if wbQuickClean then begin
        wbBuildRefs := False;
        wbForceBuildRefs := False;
      end;
      ...
      if ltLoadOrderOffset + ltLoadList.Count >= 255 then begin
        ...
        if wbBuildRefs then
          for i := Low(ltFiles) to High(ltFiles) do
            if wbForceBuildRefs or (
               not SameText(ltFiles[i].FileName, wbGameName + '.esm') and
